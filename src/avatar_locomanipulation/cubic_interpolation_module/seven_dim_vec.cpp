...
void SevenDimVec::evaluate(const double & s_global){
	...
	for(int i=0; i<(N-2); ++i){
		...
		// std::cout << "Lower Bound: ( i / ((double) (N-3)) ) = " << ( i / ((double) (N-3)) ) << std::endl;
		// std::cout << "Upper Bound: ( ((double) (i+1))/((double) (N-3)) ) = " << ( ((double) (i+1))/((double) (N-3)) ) << std::endl;
	
		// Ensure we use global s to select the proper SevenDim to use
		if( ( i / ((double) (N-3)) ) <= s_ && s_ < ( ((double) (i+1))/((double) (N-3)) ) ){

			// Use the global s to obtain the local s as used by each OneDim
			// s_local = (s_ - i/(N-3)) / (1/(N-3))
			double s_local = ( s_ - (i / ((double) (N-3))) ) / ( (1/ ((double) (N-3)) ) );
