...
TaskRkneeSelfCollision::TaskRkneeSelfCollision(std::shared_ptr<RobotModel> & input_model, const std::string & input_frame_name, std::shared_ptr<CollisionEnvironment> & collision){
...
TaskRkneeSelfCollision::~TaskRkneeSelfCollision(){
	std::cout << "[Task Rhand Self Collision] for frame " << frame_name << " Destroyed" << std::endl;
}

void TaskRkneeSelfCollision::getTaskJacobian(Eigen::MatrixXd & J_task){
	robot_model->get6DTaskJacobian(frame_name, J_task);
}
void TaskRkneeSelfCollision::getTaskJacobianDot(Eigen::MatrixXd & Jdot_task){
...
// Set Task References
void TaskRkneeSelfCollision::setReference(const Eigen::VectorXd & vec_ref_in){
	vec_ref_ = vec_ref_in;
}

void TaskRkneeSelfCollision::setReference(const Eigen::Quaterniond & quat_ref_in){
	quat_ref_ = quat_ref_in;
}


void TaskRkneeSelfCollision::setReference(const Eigen::VectorXd & vec_ref_in, const Eigen::Quaterniond & quat_ref_in){
	vec_ref_ = vec_ref_in;
	quat_ref_ = quat_ref_in;
}
...
// Get Task References
void TaskRkneeSelfCollision::getReference(Eigen::VectorXd & vec_ref_out){
	vec_ref_out = vec_ref_;
}

void TaskRkneeSelfCollision::getReference(Eigen::VectorXd & vec_ref_out, Eigen::Quaterniond & quat_ref_out){
	vec_ref_out = vec_ref_;
	quat_ref_out = quat_ref_;
}

void TaskRkneeSelfCollision::getReference(Eigen::Quaterniond & quat_ref_out){
	quat_ref_out = quat_ref_;
}

void TaskRkneeSelfCollision::computeError(){
...
void TaskRkneeSelfCollision::getError(Eigen::VectorXd & error_out, bool compute){
	if (compute){
		this->computeError();
	}
	error_out = error_;
...
void TaskRkneeSelfCollision::setError(const Eigen::VectorXd & error_in){
