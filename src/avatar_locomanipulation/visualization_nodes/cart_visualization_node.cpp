...
void CartVisualizationNode::getVizInformation(tf::Transform & tf_world_fixed, tf::Transform & tf_fixed_cart){
  ...
  if(s_current < percent_pushing){
    ...
    double x_; x_ = linear_length + ((radius + (hand_distance/2))*cos(-1.57 + (s_local*rotation_angle)));
    double y_; y_ = sin(-1.57 + (s_local*rotation_angle))*(radius + (hand_distance/2.));
    ...
    tf_fixed_cart.setOrigin(tf::Vector3(x_, y_, 0.0 ) );    
