...
void feasibility::initialize_configuration_top(){
  ...
  // THIS IS WHERE YOU CALL PINOCCHIO RANDOM CONFIG

  // FOR NOW WE CAN TEST WITH EXAMPLE CONFIG
  ...
  q_start[2] = 2.0; // set z value to 1.0, this is the pelvis location
...
void feasibility::initialize_desired_top(){
  ...
  // lfoot_des_pos[0] = std::rand() % ;
  // lfoot_des_pos[1] = .2+(std::rand()*0.5+0.5)*(0.2);
