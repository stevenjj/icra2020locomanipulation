...
int main(int argc, char **argv){
	...
	double theta_1 = 0;//M_PI/4.0;	
	Eigen::AngleAxis<double> aa(theta_1, Eigen::Vector3d(0.0, 0.0, 1.0)); // yaw pi/4 to the left	
	Eigen::Quaternion<double> quat_init_1; quat_init_1 =  aa;
	val_config[3] = quat_init_1.x();// 0.0;	
	val_config[4] = quat_init_1.y(); //0.0;
	val_config[5] = quat_init_1.z(); //sin(theta/2.0);
	val_config[6] = quat_init_1.w(); //cos(theta/2.0);

	box_config << 5, 0, 1, 0;

	cart_config[0] = 0.3;  cart_config[1] = 0.0;  cart_config[2] = 0.0;
	...
	appended_config << val_config, cart_config;
