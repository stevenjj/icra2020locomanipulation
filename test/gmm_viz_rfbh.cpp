...
int main(int argc , char ** argv){
  ...
  char filename[64];
  snprintf(filename, sizeof(char)*64, "/home/mihir/lMD/R/good/rfbh/norm.yaml");
  ...
  param_handler.getNestedValue({"Normalization Data", "rhx", "Mu"}, data_mean[6]);
  param_handler.getNestedValue({"Normalization Data", "rhx", "Sigma"}, data_std_dev[6]);
  param_handler.getNestedValue({"Normalization Data", "rhy", "Mu"}, data_mean[7]);
  param_handler.getNestedValue({"Normalization Data", "rhy", "Sigma"}, data_std_dev[7]);
  param_handler.getNestedValue({"Normalization Data", "rhz", "Mu"}, data_mean[8]);
  param_handler.getNestedValue({"Normalization Data", "rhz", "Sigma"}, data_std_dev[8]);
  param_handler.getNestedValue({"Normalization Data", "rhrx", "Mu"}, data_mean[9]);
  param_handler.getNestedValue({"Normalization Data", "rhrx", "Sigma"}, data_std_dev[9]);
  param_handler.getNestedValue({"Normalization Data", "rhry", "Mu"}, data_mean[10]);
  param_handler.getNestedValue({"Normalization Data", "rhry", "Sigma"}, data_std_dev[10]);
  param_handler.getNestedValue({"Normalization Data", "rhrz", "Mu"}, data_mean[11]);
  param_handler.getNestedValue({"Normalization Data", "rhrz", "Sigma"}, data_std_dev[11]);

  param_handler.getNestedValue({"Normalization Data", "rfx", "Mu"}, data_mean[12]);
  param_handler.getNestedValue({"Normalization Data", "rfx", "Sigma"}, data_std_dev[12]);
  param_handler.getNestedValue({"Normalization Data", "rfy", "Mu"}, data_mean[13]);
  param_handler.getNestedValue({"Normalization Data", "rfy", "Sigma"}, data_std_dev[13]);
  param_handler.getNestedValue({"Normalization Data", "rfz", "Mu"}, data_mean[14]);
  param_handler.getNestedValue({"Normalization Data", "rfz", "Sigma"}, data_std_dev[14]);
  param_handler.getNestedValue({"Normalization Data", "rfrz", "Mu"}, data_mean[15]);
  param_handler.getNestedValue({"Normalization Data", "rfrz", "Sigma"}, data_std_dev[15]);

  param_handler.getNestedValue({"Normalization Data", "comx", "Mu"}, data_mean[16]);
  param_handler.getNestedValue({"Normalization Data", "comx", "Sigma"}, data_std_dev[16]);
  param_handler.getNestedValue({"Normalization Data", "comy", "Mu"}, data_mean[17]);
  param_handler.getNestedValue({"Normalization Data", "comy", "Sigma"}, data_std_dev[17]);
  param_handler.getNestedValue({"Normalization Data", "pelz", "Mu"}, data_mean[18]);
  param_handler.getNestedValue({"Normalization Data", "pelz", "Sigma"}, data_std_dev[18]);

  double x_max = (x_max_raw-data_mean[12])/data_std_dev[12];
  double x_min = (x_min_raw-data_mean[12])/data_std_dev[12];
  double y_max = (y_max_raw-data_mean[13])/data_std_dev[13];
  double y_min = (y_min_raw-data_mean[13])/data_std_dev[13];
  ...
  for(int i = 0; i<num_clus; i++){
    param_handler.load_yaml_file(std::string("/home/mihir/lMD/R/good/rfbh/gmm") + std::to_string(i) + std::string(".yaml"));
    ...
    for(int m = 0; m < 2; m++){
      list_of_mus[i][m] = mu[m+12];
      // std::cout << "mu[" << i << "][" << m <<"]: " << list_of_mus[i][m] << std::endl;
    }
  }
  for(int i = 0; i<num_clus; i++){
    for(int k = 0; k<2; k++){
      for(int r=0; r<2; r++){
        list_of_Sigmas[i](k, r) = list_of_Sigmas_raw[i](k+12, r+12);
